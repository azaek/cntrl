...
import NetworkStat from "./network-stat";
import PowerControl from "./power-control";
import ProcessList from "./process-list";
import SystemInfo from "./system-info";
import UptimeStat from "./uptime-stat";

const BridgesControl = () => {
    const { bridges, addBridge, ready } = useBridges();

    const { statuses } = useBridgesStatus(Array.from(bridges.keys()), {
        interval: 10000,
    });

    return (
        <div className="w-full max-w-md rounded-md border p-2">
...
const BridgeItem = ({
    ...
    const { disconnect, connect, removeBridge } = useBridges();
    ...
    return (
        <div className="flex w-full flex-col">
            <div className="flex min-h-8 items-center justify-between">
                ...
                <div className="flex items-center justify-end gap-3 pr-2">
                    ...
                    {bridge.status === "disconnected" && (
                        <ZapOff className="size-4 text-red-400" />
                    )}
                    {status?.online ? (
            ...
            {/* Network (WS) */}
            <div className="mt-2">
                <NetworkStat bridgeId={bridge.id} />
            </div>

            {/* Disks (WS) */}
            <div className="mt-2">
                <DiskStat bridgeId={bridge.id} />
            </div>

            {/* Media (WS) */}
            <div className="mt-2">
                <MediaControl bridgeId={bridge.id} />
            </div>

            {/* Processes (WS) */}
            <div className="mt-2">
                <ProcessList bridgeId={bridge.id} />
            </div>

            {/* Power (REST) */}
            <div className="mt-2">
                <PowerControl bridgeId={bridge.id} />
            </div>
