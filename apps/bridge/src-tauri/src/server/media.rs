...
pub async fn get_media_status() -> Option<MediaStatus> {
    #[cfg(target_os = "windows")]
    {
                ...
                if let Ok(session) = manager.GetCurrentSession() {
                    ...
                    if let Ok(props_res) = session.TryGetMediaPropertiesAsync() {
                        if let Ok(props) = props_res.await {
                            if let Ok(t) = props.Title() { title = Some(t.to_string()); }
                            if let Ok(a) = props.Artist() { artist = Some(a.to_string()); }
                        }
                    }
                }
            }
        ...
        return Some(MediaStatus {
            ...
            muted,
            playing,
            title: if title.as_deref() == Some("") { None } else { title },
            artist: if artist.as_deref() == Some("") { None } else { artist },
            supports_ctrl: true,
        });
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let vol_out = Command::new("osascript").args(["-e", vol_script]).output().ok();
        let mut volume = None;
        let mut muted = None;
        ...
        let media_out = Command::new("osascript").args(["-e", media_script]).output().ok();
        let mut status = "stopped".to_string();
        let mut title = None;
        let mut artist = None;
        let mut playing = Some(false);
...
pub unsafe fn set_volume(volume: i32) -> Option<()> {
    ...
    volume_endpoint.SetMasterVolumeLevelScalar(vol_scalar, std::ptr::null()).ok()?;
    Some(())
}
...
pub unsafe fn set_volume(volume: i32) -> Option<()> {
    ...
    std::process::Command::new("osascript").args(["-e", &script]).status().ok()?;
    Some(())
}
...
#[cfg(target_os = "macos")]
pub unsafe fn set_mute(mute: bool) -> Option<()> {
    let script = format!("set volume output muted {}", if mute { "true" } else { "false" });
    std::process::Command::new("osascript").args(["-e", &script]).status().ok()?;
    Some(())
}

#[cfg(all(not(target_os = "windows"), not(target_os = "macos")))]
pub unsafe fn set_volume(_volume: i32) -> Option<()> { None }
#[cfg(all(not(target_os = "windows"), not(target_os = "macos")))]
pub unsafe fn set_mute(_mute: bool) -> Option<()> { None }
